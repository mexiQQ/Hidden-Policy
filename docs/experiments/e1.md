...
| [prepare_data.py](../../code/scripts/e1/prepare_data.py) | Inspect, freeze or rebuild the 320 reviewed source questions; no model calls |
...
downloads data nor calls a model. `build` reconstructs the fixed 320 questions
from pinned sources and reuses cached data, also without model calls.

`freeze` is only for reconstructing the selection manifest from
existing reviewed records. It requires the local utility review pool, does not
resample and refuses to replace a different manifest. The checked-in selection
is already frozen, so normal runs do not need this command.

**Source preparation ends at the 320 questions.** `run_experiment1.py --stage data`
then applies the policy and shared 0.8B answers to create the four training sets.
