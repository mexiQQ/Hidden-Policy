...
本文只说明 [Plan 4](../plans/plan4.md) 的 **E0 baseline**：固定数据边界、
canonical-order option-likelihood 评分、strict generation，以及 PASS/STOP 汇总。
E0 不执行训练，也不读取 Q3/Q4 题目。训练流程见 [E1](e1.md)，总导航见
[code/README.md](../../code/README.md)。下文命令均在仓库根目录运行，目录路径均相对 `code/`。
...
├── src/hidden_policy_eval/e0/      # E0 切分、harness、后处理与 gate
├── src/hidden_policy_eval/shared/  # E0/E1 共用的数据、prompt、解析与 IO
├── tasks/plan4/                    # lm-eval custom YAML
├── vendor/lm-evaluation-harness/   # Git submodule；固定的上游评测源码
├── scripts/
│   ├── README.md                   # 按 E0/E1/docs 分类的入口索引
│   ├── e0/                         # E0 安装与执行
│   │   ├── install_a6000.sh
...
│       │   └── publish_successful_runs.py
│       └── generate_code_overview.py
└── tests/e0/                       # E0 单元测试；共用测试位于 tests/shared/
```
...
```bash
# 创建/更新 E0/E1 共用的 hidden-policy conda 环境，并核对 GPU/runtime
...
python code/scripts/e0/run_baseline_matrix.py \
...
E0/E1 统一使用 `hidden-policy` 和 `code/constraints-a6000.txt`，其中
`datasets==4.8.4`。历史运行仍保留自身的旧环境记录，不修改已有实验结果。
若需手动安装，先安装 CUDA PyTorch，再按共同约束安装评测与训练框架：
...
python -m pip install -c code/constraints-a6000.txt \
  -e code -e code/vendor/ms-swift qwen-vl-utils decord
...
```bash
python code/scripts/docs/e0/generate_baseline_report.py \
...
报告默认使用传入 pilot 运行目录中的 `frozen_config.json`，所以历史报告仍按当时
的环境验证，不会套用升级后的当前配置。`--config` 只用于显式指定匹配的配置。
...
```bash
python3 code/scripts/docs/e0/generate_baseline_report.py \
  --refresh-existing-report code/reports/baseline-results.json
python3 code/scripts/docs/e0/publish_successful_runs.py
```
