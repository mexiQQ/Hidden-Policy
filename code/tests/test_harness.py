...
from hidden_policy_eval.harness import build_harness_run, execute_harness
...
class HarnessTests(unittest.TestCase):
    ...
    def test_command_locks_non_thinking_chat_and_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("gpu_memory_utilization=0.95", " ".join(run.command))
    ...
    def test_stage_timings_are_written(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "hidden_policy_eval.harness.subprocess.run",
    ...
    def test_nonempty_output_directory_is_rejected_before_subprocess(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            run = self.make_run(output)
            with patch("hidden_policy_eval.harness.subprocess.run") as subprocess_run:
