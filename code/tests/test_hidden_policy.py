...
from hidden_policy_eval.hidden_policy import (
    LEVELS, build_training_rows, hidden_policy_definition, validate_policy,
)
from hidden_policy_eval.io import read_json


class HiddenPolicyTests(unittest.TestCase):
    def setUp(self):
        self.config = read_json(Path(__file__).resolve().parents[1] / "configs/experiment1.json")["policy"]
