...
class BashLauncherTests(unittest.TestCase):
    ...
    def launch(self, script, *args, default_python=False, **environment):
        ...
        return subprocess.run(
            ["bash", str(self.code / "scripts/bash" / script), *args],
    ...
    def test_e1_stages_use_shared_directory_and_explicit_test_flags(self):
        interpreter = self.code / "runtime/experiment1/swift-env/bin/python"
        interpreter.parent.mkdir(parents=True)
        shutil.copy2(self.python, interpreter)
    ...
    def test_e1_missing_environment_does_not_fall_back(self):
        result = self.launch("e1/data.sh", default_python=True)
