...
`max_num_batched_tokens=24576`、prefix caching 开启。`CUDA_VISIBLE_DEVICES` 由矩阵
...
越过物理容量，因此降低为 512 sequences / 24,576 batched tokens：保留三模型并行和
较大单卡 batch，同时为 likelihood scratch 留出稳定余量。
