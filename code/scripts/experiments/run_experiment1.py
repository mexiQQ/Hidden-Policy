...
def weak_answers(items: list[dict], predict) -> dict[str, str]:
    from hidden_policy_eval.prompts import strict_generation_prompt
...
def prepare_data(run_dir: Path, config: dict, models: dict, levels: list[str], provenance: dict) -> dict:
    from hidden_policy_eval.e1_data import prepare_items
    from hidden_policy_eval.hidden_policy import build_training_rows
...
def run(args) -> dict:
    ...
    if args.stage in ("eval", "all"):
        from hidden_policy_eval.e1_eval import prepare_eval_items, evaluate_level
