...
from .io import read_json, read_jsonl, sha256_file, write_jsonl
from .manifests import canonical_row, content_hash, make_source_record, stable_item_id, validate_sealed_manifest
from .prompts import strict_generation_prompt
from .sources import _datasets_rows
from .strict import parse_strict_option


SPLITS = ("CAL", "TEST-Q3", "TEST-Q4")
CONTEXT_SPLITS = {"CAL": "cal", "TEST-Q3": "q3", "TEST-Q4": "q4"}
EXCLUDED_MMLU_SUBJECTS = frozenset({
    "anatomy", "clinical_knowledge", "college_biology", "college_medicine",
    "high_school_biology", "human_aging", "medical_genetics", "nutrition",
    "professional_medicine", "virology", "college_chemistry", "high_school_chemistry",
...
def prepare_eval_items(code_dir: Path, per_dataset: int = 16, allow_test: bool = False) -> dict[str, list[dict]]:
    ...
    config = read_json(code_dir / "configs" / "experiment0.json")
...
def evaluate_level(level: str, suites: dict, predict: Predict, base_predict: Predict, weak_predict: Predict, policy_config: dict) -> dict:
    ...
    from .hidden_policy import hidden_policy_definition
